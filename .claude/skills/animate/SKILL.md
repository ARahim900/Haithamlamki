...
Use `var(--duration-fast)` etc. in your `transition-duration` and `animation-duration` declarations instead of hardcoded values so reduced-motion is respected automatically without global overrides.
