          ...
          <div className="chart-wrap" style={{height:chartH}}>
            <ResponsiveContainer width="100%" height={chartH}>
              <ComposedChart data={revData}>
                ...
                <Bar dataKey="npt" fill={t.negative} opacity={0.6} name="NPT Loss ($M)" radius={[2,2,0,0]}/>
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              {fleetRows.slice(0,8).map((r,i)=>{
                ...
                return (
                  <tr key={i}>
                    ...
                    <td style={{color:"var(--color-positive)",fontWeight:700}}>{op}</td>
                    <td style={{fontWeight:700}}>{(op/24).toFixed(1)}</td>
                    <td style={{color:rd?"var(--color-warning)":"var(--color-text-muted)",fontWeight:rd?700:400}}>{rd||"-"}</td>
                    <td style={{color:"var(--color-text-muted)"}}>-</td>
                    <td style={{color:sp?"var(--color-info)":"var(--color-text-muted)",fontWeight:sp?700:400}}>{sp||"-"}</td>
                    <td style={{color:"var(--color-text-muted)"}}>-</td>
                    <td style={{color:"var(--color-text-muted)"}}>-</td>
                    <td style={{fontWeight:700}}>744</td>
                    <td style={{fontWeight:600,color:"var(--color-primary)"}}>${Math.round(rev).toLocaleString()}</td>
                  </tr>
                );
              })}
            </tbody>
